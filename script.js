document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchCakes(showIndicator = true) {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                // Filter out completely empty rows if they exist in the sheet
                const validData = data.filter(item => item && (item.name || item.Name || item.rowIndex));
                
                cakes = validData.map((item, i) => ({
                    rowIndex: item.rowIndex || (i + 2),
                    name    : item.name     || item.Name     || "Signature Creation",
    ...
    function renderProducts(filter = null) {
        ...
        const activeFilter = (filter || document.querySelector('.filter-btn.active')?.dataset.filter || 'all').toLowerCase();
        ...
        filtered.forEach((cake, index) => {
            ...
            gsap.from(card, { 
                ...
                scale: 0.95,
                duration: 0.8, 
                delay: Math.min(index * 0.05, 1.2), // Cap the delay to 1.2s max
                ease: "power2.out" 
            });
        });
